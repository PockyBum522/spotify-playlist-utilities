...
{
    private string _credentialsPath => Path.Join(Path.GetDirectoryName(Environment.ProcessPath) ?? "ERROR_GETTING_APP_PATH", "credentials.json");
    ...
    public async Task<SpotifyClient> getAuthenticatedSpotifyClient()
    {
        ...
        if (!File.Exists(_credentialsPath))
        {
            throw new AuthenticationException(
                $"Could not find credentials file after authentication: {_credentialsPath}");
        }
        ...
        authenticator.TokenRefreshed += (_, refreshedToken) => File.WriteAllText(_credentialsPath, JsonConvert.SerializeObject(refreshedToken));
